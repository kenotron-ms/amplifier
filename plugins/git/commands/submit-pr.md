...
if [[ -f ".git-pr-config.json" ]]; then
  echo "🛡️  Found PR safety config - validating target branch..."

  # Read the config (requires jq)
  if command -v jq &> /dev/null; then
    ALLOWED_BRANCHES=$(jq -r '.rules.allowed_base_branches[]' .git-pr-config.json 2>/dev/null)
    ...
    if git rev-parse --verify origin/main >/dev/null 2>&1; then
      ...
      # Try to detect from allowed branches
      for branch in $ALLOWED_BRANCHES; do
        if git rev-parse --verify origin/$branch >/dev/null 2>&1; then
          DETECTED_BASE="$branch"
          break
        fi
      done
    fi

    # Check if detected base is forbidden
    if echo "$FORBIDDEN_BRANCHES" | grep -q "^${DETECTED_BASE}$"; then
      echo "❌ ERROR: Cannot create PR to branch '$DETECTED_BASE'"
      echo ""
      echo "This repository has PR safety rules configured."
      echo ""
      echo "Forbidden base branches:"
      echo "$FORBIDDEN_BRANCHES" | sed 's/^/  - /'
      echo ""
      echo "Allowed base branches:"
      echo "$ALLOWED_BRANCHES" | sed 's/^/  - /'
      echo ""
      echo "Reason (from config):"
      jq -r '.rules.notes[]' .git-pr-config.json 2>/dev/null | sed 's/^/  /'
      echo ""
      echo "Please ensure you're working on an allowed base branch."
      exit 1
    fi

    # If detected base is in allowed list, proceed
    if echo "$ALLOWED_BRANCHES" | grep -q "^${DETECTED_BASE}$"; then
      echo "✅ PR target validated: '$DETECTED_BASE' is allowed"
    else
      echo "⚠️  Warning: Current base branch '$DETECTED_BASE' not in allowed list"
      echo "   Allowed branches: $ALLOWED_BRANCHES"
      echo "   Proceeding with caution..."
    fi
  else
    echo "⚠️  jq not installed - skipping PR safety validation"
    echo "   Install jq to enable PR target branch validation"
  fi
  echo ""
fi
```
